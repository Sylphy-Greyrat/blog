...
- Servlet 的作用：它是基于 Http 协议的，主要是用来接受客户端发送的请求，处理请求，把数据响应给客户端。
  ...
  ![Servlet 的用途](http://www.ozh.asia:9000/blog/Servlet的用途.png)
...
4. 第二个 Servlet 处理完数据后将最终处理结果返回给客户端。
   ...
   具体过程如图所示：![request 实现请求转发的原理](http://www.ozh.asia:9000/blog/request实现请求转发的原理.png)
...
1. 重定向的工作流程如下：
   ...
   4. 客户端收到响应结果后，从 HTTP 响应头中取得相关信息，再次发送请求数据给服务器。
      ...
      具体过程如图所示：![response 实现重定向的原理](http://www.ozh.asia:9000/blog/response实现重定向的原理.png)
...
- Servlet的生命分为以下四部分：Servlet实例化 --> 初始化 --> 服务 --> 销毁
  ...
  4. 销毁：对应 destroy 方法。destroy() 方法仅执行一次，只在 Web 服务器端停止并卸载 Servlet 时执行。当 Servlet 对象被销毁时，并释放其占用的资源。
     ...
     ![http 请求的过程](http://www.ozh.asia:9000/blog/http请求的过程.png)
...
- Servlet 生命周期的各个阶段
  ...
  ![Servlet 生命周期](http://www.ozh.asia:9000/blog/Servlet生命周期.png)
