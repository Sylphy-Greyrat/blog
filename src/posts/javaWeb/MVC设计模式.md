...
MVC模式是Web开发中最常用的一种设计模式，规范了Web数据流的流程，通过数据、视图以及控制分离的方法来组织代码，提高了软件的灵活性和复用性，同时使程序具有对象化特征，也更容易维护。
...
- MVC设计模式流程
  ...
  ![295ddcc75ad2c2f7f9bfea66992216dc](./img/295ddcc75ad2c2f7f9bfea66992216dc.png)

  MVC模式各个组件所对应的技术如下图所示：

  ![1a378576a4fb920dc5e6a7b21cea60e8](./img/1a378576a4fb920dc5e6a7b21cea60e8.png)
...
- EL在开发中，通常是用来获取域对象中保存的值，基本语法：${域对象的名称}。
   ...
   如果域对象的名称写错了，使用EL表达式获取值，获取的是""而不会获取NULL。

### EL表达式的标识符

   在EL书写过程中，会使用一些符号来标记变量、函数名等，这些符号称之为标识符。
   书写规范：
...
### EL中的常量
   ...
   1. 布尔常量：true 或 false
   2. 数字常量：整型、浮点常量，使用方法与java相似
   3. 字符串常量：使用方法与 java 相似，比如：${"EL的字符串常量"}
   4. NULL常量：${null}
...
### EL中的隐式对象
   ...
   2. web域相关的对象
      ...
      EL表达式获取域对象中的值：如果域对象的名称相同，获取的是域对象作用范围最小的值。
...
2. 将 jstl 相应的 jar 包添加到工程 lib 目录下。
   ...
   ![image-20230629152315967](./img/22.png)

3. 在 IDEA 中配置 JSTL 的 tld 模板，***URL:```http://java.sun.com/jsp/jstl/core```。***

   ![image-20230629152315967](./img/6.png)

4. 添加 JSTL jar 包，修复 artifact 的错误。

   ![image-20230629152315967](./img/e.png)
...
3. \<c:choose>标签：***进行条件判断，和 java 类中的 if ,else if(),else if()……else相似***
