...
1. 同步调用
   ...
   ![异步调用的展示](http://www.ozh.asia:9000/blog/异步调用的展示.png)
...
![RabbitMQ 框架](http://www.ozh.asia:9000/blog/RabbitMQ框架.png)
...
![rabbitmq管理界面的Exchange选项卡](http://www.ozh.asia:9000/blog/rabbitmq管理界面的Exchange选项卡.png)

​	点击任意交换机，即可进入交换机详情页面。（可利用控制台的 publish message 发送一天消息）：

![exchanges的详情页](http://www.ozh.asia:9000/blog/exchanges的详情页.png)

![消息发送的具体操作](http://www.ozh.asia:9000/blog/消息发送的具体操作.png)
...
![rabbitmq管理界面的Queues选项卡](http://www.ozh.asia:9000/blog/rabbitmq管理界面的Queues选项卡.png)

命名为 hello.queue1：

![创建hello.queue1队列的操作](http://www.ozh.asia:9000/blog/创建hello.queue1队列的操作.png)

再以相同的方式，创建一个队列，密码为 hello.queue2，最终队列列表如下：

![队列列表](http://www.ozh.asia:9000/blog/队列列表.png)
...
![绑定操作](http://www.ozh.asia:9000/blog/绑定操作.png)

相同的方式，将 hello.queue2 也绑定到改交换机。 最终，绑定结果如下：

![绑定结果](http://www.ozh.asia:9000/blog/绑定结果.png)
...
![消息结果](http://www.ozh.asia:9000/blog/消息结果.png)

点击队列名称，进入详情页，查看队列详情，这次我们点击 get message：

![查看消息详情](http://www.ozh.asia:9000/blog/查看消息详情.png)

可以看到消息到达队列了：

![消息内容](http://www.ozh.asia:9000/blog/消息内容.png)
...
![rabbitmq管理界面的Admin选项卡](http://www.ozh.asia:9000/blog/rabbitmq管理界面的Admin选项卡.png)
...
![创建用户](http://www.ozh.asia:9000/blog/创建用户.png)

发现 hmall 用户此时没有任何 virtual host 的访问权限：

![hmall 用户的 virtualhost 访问权限](http://www.ozh.asia:9000/blog/hmall用户的virtualhost访问权限1.png)

#### virtual host

先登出用户：

![登出用户](http://www.ozh.asia:9000/blog/登出用户.png)

​	切换到刚刚创建的 hmall 用户登录，然后点击 Virtual Hosts 菜单，进入 virtual host 管理页：

![virtual host 管理页](http://www.ozh.asia:9000/blog/virtualhost管理页1.png)

​	可以看到目前只有一个默认的 virtual host ，名字为 /。 我们可以给黑马商城项目创建一个单独的 virtual host ，而不是使用默认的 /。

![创建 virtual host](http://www.ozh.asia:9000/blog/创建virtualhost.png)

创建完成后如图：

![virtual host 管理页](http://www.ozh.asia:9000/blog/virtualhost管理页2.png)

​	由于我们是登录 hmall 账户后创建的 virtual host，因此回到 users 菜单，你会发现当前用户已经具备了对 /hmall 这个 virtual host 的访问权限了：

![hmall 用户的 virtualhost 访问权限](http://www.ozh.asia:9000/blog/hmall用户的virtualhost访问权限2.png)

此时，点击页面右上角的 virtual host 下拉菜单，切换 virtual host 为  /hmall：

![切换 virtual host](http://www.ozh.asia:9000/blog/切换virtualhost.png)
...
![跳过交换机的消息流程](http://www.ozh.asia:9000/blog/跳过交换机的消息流程.png)
...
![](http://www.ozh.asia:9000/blog/新建simple.queue队列1.png)

![新建 simple.queue 队列](http://www.ozh.asia:9000/blog/新建simple.queue队列2.png)
...
![消息传到队列中](http://www.ozh.asia:9000/blog/消息传到队列中.png)
...
![运行结果](http://www.ozh.asia:9000/blog/运行结果1.png)

### WorkQueues 模型

​	Work queues，任务模型。简单来说就是**让多个消费者绑定到一个队列，共同消费队列中的消息**。

![Work Queues 模型的消息流程](http://www.ozh.asia:9000/blog/WorkQueues模型的消息流程.png)
...
![新建 work.queue 队列](http://www.ozh.asia:9000/blog/新建work.queue队列.png)
...
![包含交换机的消息流程](http://www.ozh.asia:9000/blog/包含交换机的消息流程.png)
...
![Fanout 交换机的消息流程](http://www.ozh.asia:9000/blog/Fanout交换机的消息流程.png)
...
![创建 fanout.queue1 队列](http://www.ozh.asia:9000/blog/创建fanout.queue1队列.png)

在控制台创建队列 fanout.queue2:

![创建 fanout.queue2 队列](http://www.ozh.asia:9000/blog/创建fanout.queue2队列.png)

然后再创建一个交换机：

![创建 hmall.fanout 交换机](http://www.ozh.asia:9000/blog/创建hmall.fanout交换机.png)

然后绑定两个队列到交换机：

![fanout.queue1绑定到hmall.fanout](http://www.ozh.asia:9000/blog/fanout.queue1_bind_hmall.fanout.png)

![fanout.queue2绑定到hmall.fanout](http://www.ozh.asia:9000/blog/fanout.queue2_bind_hmall.fanout.png)
...
![Direct 交换机的消息流程](http://www.ozh.asia:9000/blog/Direct交换机的消息流程.png)
...
![创建direct.queue1和direct.queue2两个队列](http://www.ozh.asia:9000/blog/创建direct.queue1和direct.queue2两个队列.png)

然后声明一个direct类型的交换机，命名为 hmall.direct：

![创建hmall.direct交换机](http://www.ozh.asia:9000/blog/创建hmall.direct交换机.png)

然后使用 red 和 blue 作为 key，绑定 direct.queue1 到 hmall.direct：

![direct.queue1 绑定 hmall.direct 用 red 做 key](http://www.ozh.asia:9000/blog/direct.queue1绑定hmall.direct用red做key.png)

![direct.queue1绑定hmall.direct用blue做key](http://www.ozh.asia:9000/blog/direct.queue1绑定hmall.direct用blue做key.png)

​	同理，使用 red 和 yellow 作为 key，绑定 direct.queue2 到 hmall.direct，步骤略，最终结果：

![hmall.direct 的绑定情况](http://www.ozh.asia:9000/blog/hmall.direct的绑定情况.png)
...
![发送 key 为 red 的消息的运行结果](http://www.ozh.asia:9000/blog/发送key为red的消息的运行结果.png)
...
![发送key为blue的消息的运行结果](http://www.ozh.asia:9000/blog/发送key为blue的消息的运行结果.png)
...
![Topic 交换机的消息流程](http://www.ozh.asia:9000/blog/Topic交换机的消息流程.png)

![Topic 交换机的例子](http://www.ozh.asia:9000/blog/Topic交换机的例子.png)

##### 声明队列和交换机

​	首先，在控制台按照图示例子创建队列、交换机，并利用通配符绑定队列和交换机。此处步骤略。最终结果如下：

![hmall.topic 的绑定情况](http://www.ozh.asia:9000/blog/hmall.topic的绑定情况.png)
...
![Queue 类](http://www.ozh.asia:9000/blog/Queue类.png)

SpringAMQP 还提供了一个 Exchange 接口，来表示所有不同类型的交换机：

![Exchange 接口及其子类](http://www.ozh.asia:9000/blog/Exchange接口及其子类.png)

​	我们可以自己创建队列和交换机，不过 SpringAMQP 还提供了 <font color='red'>**ExchangeBuilder**</font>来简化这个过程： 

![ExchangeBuilder 的结构](http://www.ozh.asia:9000/blog/ExchangeBuilder的结构.png)

而在绑定队列和交换机时，则需要使用BindingBuilder来创建Binding对象：

![BindingBuilder 的结构](http://www.ozh.asia:9000/blog/BindingBuilder的结构.png)
...
![convertAndSend 方法](http://www.ozh.asia:9000/blog/convertAndSend方法.png)
...
![收到的消息的消息结构](http://www.ozh.asia:9000/blog/收到的消息的消息结构.png)
...
![消息推送流程](http://www.ozh.asia:9000/blog/消息推送流程.png)
...
![消息处理返回的不同情况](http://www.ozh.asia:9000/blog/消息处理返回的不同情况.png)
...
![publichser 模块结构](http://www.ozh.asia:9000/blog/publichser模块结构.png)
...
![convertAndSend 的多种构造](http://www.ozh.asia:9000/blog/convertAndSend的多种构造.png)
...
![SettableListenableFuture 类](http://www.ozh.asia:9000/blog/SettableListenableFuture类.png)
...
![添加 ConfirmCallback 的运行结果](http://www.ozh.asia:9000/blog/添加ConfirmCallback的运行结果.png)
...
![comfirm 模式的流程](http://www.ozh.asia:9000/blog/comfirm模式的流程.png)
...
![confirm 模式细节处理](http://www.ozh.asia:9000/blog/confirm模式细节处理.png)
...
![Exchanges 配置 Druability 参数](http://www.ozh.asia:9000/blog/Exchanges配置Druability参数.png)
...
![Queues 配置 Druability 参数](http://www.ozh.asia:9000/blog/Queues配置Druability参数.png)
...
![消息持久化配置](http://www.ozh.asia:9000/blog/消息持久化配置.png)
...
![](http://www.ozh.asia:9000/blog/写进磁盘的非持久化消息.png)
...
![Queues 配置 Lazy 模式](http://www.ozh.asia:9000/blog/Queues配置Lazy模式.png)
...
![lazy() 源码](http://www.ozh.asia:9000/blog/lazy()源码.png)
...
![Admin 页面配置全部队列为 Lazy](http://www.ozh.asia:9000/blog/Admin页面配置全部队列为Lazy.png)
...
![队列显示 unacked(未确定状态)](http://www.ozh.asia:9000/blog/队列显示unacked(未确定状态).png)

​	放行以后，由于抛出的是**消息转换异常**，因此 Spring 会自动返回 reject，所以消息依然会被删除：

![队列消息被删除](http://www.ozh.asia:9000/blog/队列消息被删除.png)
...
![消息恢复至 Ready 状态](http://www.ozh.asia:9000/blog/消息恢复至Ready状态.png)
...
![兜底方案的流程](http://www.ozh.asia:9000/blog/兜底方案的流程.png)
...
![备份交换机的原理步骤](http://www.ozh.asia:9000/blog/备份交换机的原理步骤.png)
...
![](http://www.ozh.asia:9000/blog/延迟消息流程5.png)
...
![设置代码](http://www.ozh.asia:9000/blog/设置代码.png)
...
![订单状态同步思路](http://www.ozh.asia:9000/blog/订单状态同步思路.png)
...
![优化后的订单状态同步思路](http://www.ozh.asia:9000/blog/优化后的订单状态同步思路.png)
...
![nacos 共享配置 rabbitmq](http://www.ozh.asia:9000/blog/nacos共享配置rabbitmq.png)
