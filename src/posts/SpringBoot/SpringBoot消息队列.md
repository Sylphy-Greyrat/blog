...
![RabbitMQ 框架](./img/RabbitMQ框架.png)
...
![rabbitmq管理界面的Exchange选项卡](./img/rabbitmq管理界面的Exchange选项卡.png)

​	点击任意交换机，即可进入交换机详情页面。（可利用控制台的 publish message 发送一天消息）：

![exchanges的详情页](./img/exchanges的详情页.png)

![消息发送的具体操作](./img/消息发送的具体操作.png)
...
![rabbitmq管理界面的Queues选项卡](./img/rabbitmq管理界面的Queues选项卡.png)

命名为 hello.queue1：

![创建hello.queue1队列的操作](./img/创建hello.queue1队列的操作.png)
...
![rabbitmq管理界面的Admin选项卡](./img/rabbitmq管理界面的Admin选项卡.png)
...
![hmall 用户的 virtualhost 访问权限](./img/hmall用户的virtualhost访问权限1.png)
...
![](./img/新建simple.queue队列1.png)

![新建 simple.queue 队列](./img/新建simple.queue队列2.png)
...
![Work Queues 模型的消息流程](./img/WorkQueues模型的消息流程.png)
...
![新建 work.queue 队列](./img/新建work.queue队列.png)
...
![Fanout 交换机的消息流程](./img/Fanout交换机的消息流程.png)
...
![创建 fanout.queue1 队列](./img/创建fanout.queue1队列.png)

在控制台创建队列 fanout.queue2:

![创建 fanout.queue2 队列](./img/创建fanout.queue2队列.png)

然后再创建一个交换机：

![创建 hmall.fanout 交换机](./img/创建hmall.fanout交换机.png)

然后绑定两个队列到交换机：

![fanout.queue1绑定到hmall.fanout](./img/fanout.queue1_bind_hmall.fanout.png)

![fanout.queue2绑定到hmall.fanout](./img/fanout.queue2_bind_hmall.fanout.png)
...
![Direct 交换机的消息流程](./img/Direct交换机的消息流程.png)
...
![创建direct.queue1和direct.queue2两个队列](./img/创建direct.queue1和direct.queue2两个队列.png)

然后声明一个direct类型的交换机，命名为 hmall.direct：

![创建hmall.direct交换机](./img/创建hmall.direct交换机.png)

然后使用 red 和 blue 作为 key，绑定 direct.queue1 到 hmall.direct：

![direct.queue1 绑定 hmall.direct 用 red 做 key](./img/direct.queue1绑定hmall.direct用red做key.png)

![direct.queue1绑定hmall.direct用blue做key](./img/direct.queue1绑定hmall.direct用blue做key.png)

​	同理，使用 red 和 yellow 作为 key，绑定 direct.queue2 到 hmall.direct，步骤略，最终结果：

![hmall.direct 的绑定情况](./img/hmall.direct的绑定情况.png)
...
![发送 key 为 red 的消息的运行结果](./img/发送key为red的消息的运行结果.png)
...
![发送key为blue的消息的运行结果](./img/发送key为blue的消息的运行结果.png)
...
![Topic 交换机的消息流程](./img/Topic交换机的消息流程.png)

![Topic 交换机的例子](./img/Topic交换机的例子.png)

##### 声明队列和交换机

​	首先，在控制台按照图示例子创建队列、交换机，并利用通配符绑定队列和交换机。此处步骤略。最终结果如下：

![hmall.topic 的绑定情况](./img/hmall.topic的绑定情况.png)
...
![Queue 类](./img/Queue类.png)

SpringAMQP 还提供了一个 Exchange 接口，来表示所有不同类型的交换机：

![Exchange 接口及其子类](./img/Exchange接口及其子类.png)

​	我们可以自己创建队列和交换机，不过 SpringAMQP 还提供了 <font color='red'>**ExchangeBuilder**</font>来简化这个过程： 

![ExchangeBuilder 的结构](./img/ExchangeBuilder的结构.png)

而在绑定队列和交换机时，则需要使用BindingBuilder来创建Binding对象：

![BindingBuilder 的结构](./img/BindingBuilder的结构.png)
...
![convertAndSend 方法](./img/convertAndSend方法.png)
...
![消息处理返回的不同情况](./img/消息处理返回的不同情况.png)
...
![publichser 模块结构](./img/publichser模块结构.png)
...
![convertAndSend 的多种构造](./img/convertAndSend的多种构造.png)
...
![SettableListenableFuture 类](./img/SettableListenableFuture类.png)
...
![添加 ConfirmCallback 的运行结果](./img/添加ConfirmCallback的运行结果.png)
...
![comfirm 模式的流程](./img/comfirm模式的流程.png)
...
![confirm 模式细节处理](./img/confirm模式细节处理.png)
...
![Exchanges 配置 Druability 参数](./img/Exchanges配置Druability参数.png)
...
![Queues 配置 Druability 参数](./img/Queues配置Druability参数.png)
...
![Queues 配置 Lazy 模式](./img/Queues配置Lazy模式.png)
...
![Admin 页面配置全部队列为 Lazy](./img/Admin页面配置全部队列为Lazy.png)
...
![队列显示 unacked(未确定状态)](./img/队列显示unacked(未确定状态).png)

​	放行以后，由于抛出的是**消息转换异常**，因此 Spring 会自动返回 reject，所以消息依然会被删除：

![队列消息被删除](./img/队列消息被删除.png)
...
![消息恢复至 Ready 状态](./img/消息恢复至Ready状态.png)
...
![优化后的订单状态同步思路](./img/优化后的订单状态同步思路.png)
...
![nacos 共享配置 rabbitmq](./img/nacos共享配置rabbitmq.png)
