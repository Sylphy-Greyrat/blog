...
- ***单例模式是指在内存中只会创建且仅创建⼀次对象的设计模式***。在程序中***多次使用同⼀个对象且作⽤相同*** 时，为了防⽌频繁地创建对象使得内存飙升，单例模式可以让程序仅在内存中***创建⼀个对象***，让所有需要调⽤的地⽅都共享 这⼀单例对象。
  ...
  ![单例模式的概述](http://minio.ozh.asia/blog/单例模式的概述.png)
...
![Spring 的框架](http://minio.ozh.asia/blog/Spring的框架.png)
...
- 依赖注入（dependency injection）:**实现对象之间的依赖关系。** 在创建完对象之后，对象之间的关系处理就是依赖注入。
  ...
  ![依赖注入例子](http://minio.ozh.asia/blog/依赖注入例子.png)
...
- IOC 工作过程
  ...
  ![IOC 工作过程](http://minio.ozh.asia/blog/IOC工作过程.png)
...
![依赖注入的实现方式](http://minio.ozh.asia/blog/依赖注入的实现方式.png)
